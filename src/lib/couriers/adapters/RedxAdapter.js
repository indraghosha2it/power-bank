...
// // Note: RedX API endpoints may vary - adjust based on actual RedX API documentation
// const REDX_API_BASE = 'https://api.redx.com.bd/api/v1'; // Placeholder URL
...
//     try {
//       // RedX authentication - adjust based on actual RedX API
//       const response = await fetch(`${REDX_API_BASE}/auth/login`, {
//         method: 'POST',
...
//       if (!response.ok) {
//         throw new Error(data?.message || 'RedX authentication failed');
//       }
...
//     try {
//       await this.login();
//       return { success: true, message: 'RedX API connected successfully' };
//     } catch (error) {
...
//       const redxOrderData = this.formatOrderData(orderData);

//       const response = await fetch(`${REDX_API_BASE}/orders`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
//           'Authorization': `Bearer ${token}`,
//         },
//         body: JSON.stringify(redxOrderData),
//       });

//       const data = await response.json();
//       if (!response.ok) {
//         throw new Error(data?.message || 'RedX order creation failed');
//       }
...
//         trackingNumber: orderInfo.tracking_number || orderInfo.consignment_id,
//         trackingUrl: orderInfo.tracking_url || '',
...
//       item_description: order.items.map(item => `${item.productName} x${item.quantity}`).join(', ').slice(0, 255),
...
//     try {
//       const token = await this.login();
//       const response = await fetch(`${REDX_API_BASE}/orders/${trackingNumber}/tracking`, {
//         headers: { 'Authorization': `Bearer ${token}` },
//       });
//       const data = await response.json();
//       if (!response.ok) throw new Error(data?.message || 'Failed to get tracking info');
//       return { success: true, status: data.status, fullResponse: data };
//     } catch (error) {
//       return { success: false, message: error.message };
//     }
//   }
...
//     try {
//       const token = await this.login();
//       const response = await fetch(`${REDX_API_BASE}/orders/${courierOrderId}/cancel`, {
//         method: 'POST',
//         headers: { 'Authorization': `Bearer ${token}` },
//       });
//       const data = await response.json();
//       if (!response.ok) throw new Error(data?.message || 'Failed to cancel order');
//       return { success: true, message: 'Order cancelled successfully', fullResponse: data };
//     } catch (error) {
//       return { success: false, message: error.message };
//     }
//   }
