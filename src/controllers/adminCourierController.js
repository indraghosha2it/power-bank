    ...
    const courierStatus = await Promise.all(
      couriers.map(async (courier) => {
        ...
        return {
          ...
          configured: integration?.configured || false,
          apiEnabled: courier.apiEnabled,
          // 🆕 Include webhook config
          webhookConfig: courier.webhookConfig || {
            enabled: false,
            secret: null,
            bearerToken: null,
            token: null,
...
// ========== UPDATE COURIER INTEGRATION - FIXED ==========
const updateCourierIntegration = async (req, res) => {
  try {
    const { id } = req.params;
    const { 
      ...
      credentials, 
      storeConfig,
      webhookConfig  // 🆕 Add this
    } = req.body || {};

    console.log('📥 Updating courier integration:', {
      id,
      apiEnabled,
    ...
    // First, save credentials using existing function
    const courier = await saveCourierCredentials(id, {
      apiEnabled,
      credentials,
      storeConfig
    });
    ...
    // ========== 🆕 UPDATE WEBHOOK CONFIG SEPARATELY ==========
    if (webhookConfig !== undefined) {
      console.log('📥 Updating webhook config:', webhookConfig);
      ...
      courier.webhookConfig = {
        ...
        secret: webhookConfig.secret || courier.webhookConfig?.secret || null,
        bearerToken: webhookConfig.bearerToken || courier.webhookConfig?.bearerToken || null,
        token: webhookConfig.token || courier.webhookConfig?.token || null,
        events: webhookConfig.events || courier.webhookConfig?.events || []
      };

      await courier.save();
      console.log('✅ Webhook config saved:', {
        enabled: courier.webhookConfig.enabled,
    ...
    const response = courier.toObject();
    delete response.credentialsEncrypted;
